...
static size_t mtfEncode(uint8_t *dict, const size_t dictLen, uint8_t *buf, const size_t bufLen) {
    uint8_t* cursor = buf;
    for (unsigned int i = 0; i < bufLen; i++) {
...
size_t mtfCrleEncode(uint8_t *dict, size_t dictLen, uint8_t *buf, const size_t bufLen)
{
    const size_t mtfEncodedLength = mtfEncode(dict, dictLen, buf, bufLen);
    return cRleEncode(buf, mtfEncodedLength);
}

size_t mtfCrleDecode(uint8_t *dict, const uint8_t *source, const size_t sourceBufLen, uint8_t *dest)
{
